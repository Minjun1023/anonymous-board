    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                // 누구나 접근 가능
                .requestMatchers("/", "/login", "/signup", "/find-id", "/reset-password", "/new-password", "/auth/**", "/oauth2/**", "/login/oauth2/**",
                               "/api/users/signup", "/api/users/login", "/api/users/find-id", "/api/users/reset-password", "/api/users/reset-password-confirm", 
                               "/api/users/check-username", "/api/users/check-email", "/api/users/check-nickname",
                               "/api/email/**",
                               "/error", "/css/**", "/js/**", "/images/**", "/posts", "/posts/**",
                               "/static/**", "/*.html").permitAll()
                // 조회만 허용
            ...
            .logout(logout -> logout
                ...
                .logoutSuccessUrl("/")
                .deleteCookies("access_token", "jwt_token", "refreshToken", "JSESSIONID")
